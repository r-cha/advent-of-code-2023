...
let rec create_subgame kvs game =
    match kvs with
    | [] -> game
    | (color, amount) :: t -> match color with
...
let update_game game revelation =
    let new_red = max game.red revelation.red in
    let new_green = max game.green revelation.green in
    let new_blue = max game.blue revelation.blue in
    {id = game.id; red = new_red; green = new_green; blue = new_blue}
...
let process_game (line: string) id_fun =
    ...
    let processed = process_revelations revelations {empty_game with id = id} in
